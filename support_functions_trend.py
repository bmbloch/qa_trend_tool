...
def auto_rebench_check(data_temp, curryr, currmon, sector_val, avail_check, mrent_check, merent_check):
    ...
    dataframe = dataframe[((abs(dataframe['vac_diff']) >= 0.01) & (round(dataframe['vac'],4) == round(dataframe['vac_oob'],4)) & ((abs(dataframe['vac_chg'] - dataframe['rol_vac_chg']) >= 0.005) | (dataframe['yr'] < curryr) | (dataframe['currmon'] <= no_trend_mon))) | (abs(dataframe['vac'] - dataframe['rol_vac']) > 0.01)]
