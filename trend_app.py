...
def finalize_econ(confirm_click, sector_val, curryr, currmon, success_init):
    ...
    if sector_val is None or success_init == False:
        ...
        if alert_display == False:
            ...
            rebench_log['v_user'] = np.where((rebench_log['include_check_vac_1'] == 0) | (rebench_log['include_check_vac_2'] == 0), np.nan, rebench_log['v_user'])
            rebench_log['g_user'] = np.where((rebench_log['include_check_mrent_1'] == 0) | (rebench_log['include_check_mrent_2'] == 0), np.nan, rebench_log['g_user'])
            rebench_log['e_user'] = np.where((rebench_log['include_check_merent_1'] == 0) | (rebench_log['include_check_merent_2'] == 0), np.nan, rebench_log['e_user'])
