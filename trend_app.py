...
def update_decision_log(decision_data, data, drop_val, sector_val, curryr, currmon, user, action, flag_name, cons_c, avail_c, mrent_c, erent_c):
    if action == "submit":
        ...
        comments = comments[['inv_cons_comment', 'avail_comment', 'mrent_comment', 'erent_comment']]
        if sector_val != "ind":
            update_data = update_data[['identity', 'subsector', 'metcode', 'subid', 'yr', 'currmon', 'inv', 'cons', 'conv', 'demo', 'vac', 'abs', 'G_mrent', 'G_merent', 'gap', 'avail', 'mrent', 'merent', 'vac_chg']]
        else:
            update_data = update_data[['identity', 'subsector', 'metcode', 'subid', 'yr', 'currmon', 'inv', 'cons', 'vac', 'abs', 'G_mrent', 'G_merent', 'gap', 'avail', 'mrent', 'merent', 'vac_chg']]
        ...
        decision_data_test = decision_data_test.drop(['i_user', 'c_user', 'v_user', 'g_user', 'e_user', 'inv_cons_comment', 'avail_comment',
                                                      'mrent_comment', 'erent_comment', 'skipped', 'skip_user', 'rol_inv', 'rol_vac', 'rol_mrent', 'rol_merent', 'rol_vac_chg', 'rol_G_mrent', 'rol_G_merent', 'qrol_vac', 'qrol_mrent', 'qrol_merent', 
                                                      'rol_cons', 'occ'], axis=1)
        ...
        update_data[list(update_data.columns)] = np.where(update_data[list(update_data.columns)] == 9999999, np.nan, update_data[list(update_data.columns)])
        ...
        for index, row in update_data.iterrows():
            if math.isnan(row['inv']) == False:
                if update_data['i_user'].loc[index] == "Cons Auto Rebench":
                    update_data.loc[index, 'i_user'] = "Cons Auto Rebench, " + user
                else:
                    update_data.loc[index, 'i_user'] = user
            if math.isnan(row['cons']) == False:
                if update_data['c_user'].loc[index] == "Cons Auto Rebench":
                    update_data.loc[index, 'c_user'] = "Cons Auto Rebench, " + user
                else:
        ...
        for x, y in zip(replace_list + ['i_user', 'c_user', 'v_user', 'g_user', 'e_user'], drop_list + ['i_user', 'c_user', 'v_user', 'g_user', 'e_user']):
            for index, row in update_data.iterrows():
                if x in ['i_user', 'c_user', 'v_user', 'g_user', 'e_user']:
                    if row[x] != "temp":
        ...
        if has_diff == 1:
            ...
            if "trunc" in expand and "full" not in expand:
                if currmon == 1:
                    ...
                    priormon = currmon - 1
                preview_data = preview_data[(preview_data['yr'] == curryr) | ((preview_data['yr'] == curryr - 1) & ((preview_data['currmon'] >= priormon) | (currmon == 1)))| ((preview_data['yr'] == curryr - 2) & (preview_data['currmon'] == 12) & (currmon == 1))]
...
def process_init_file(sector_val, curryr, currmon, yes_refresh, no_refresh, file_used, flag_cols, msq_load):
    ...
    if sector_val is None or no_refresh == 1:
        ...
        if yes_refresh == 1:
            ...
            orig_decision_data = pd.read_pickle(Path("{}central/square/data/zzz-bb-test2/python/trend/{}/{}m{}/OutputFiles/decision_log_{}.pickle".format(get_home(), sector_val, str(curryr), str(currmon), sector_val)))
        ...
        if file_used == "oob":
            ...
            decision_data = decision_data[['identity_row', 'identity', 'subsector', 'metcode', 'subid', 'yr', 'currmon'] + oob_cols + ['rol_inv', 'rol_vac', 'rol_mrent', 'rol_merent', 'rol_cons', 'rol_vac_chg', 'rol_G_mrent', 'rol_G_merent', 'qrol_vac', 'qrol_mrent', 'qrol_merent']]
        ...
        if alert_display == False:
            ...
            if sector_val == "ind":
                ...
                for subsector in ["N", "C", "NC", "N_C_NC"]:
                    ...
                    finalized_copy.to_csv(file_path, index=False, na_rep='')
                    if subsector == "C":
                        file_path_dta = "{}/central/square/data/ret/trends-experimental/retsubtrnds_C_QUANtified_{}m{}.dta".format(get_home(), str(curryr), str(currmon))
            ...
            rebench_log = rebench_log[['identity', 'subsector', 'metcode', 'subid', 'yr', 'currmon', 'rol_inv', 'rol_vac', 'rol_mrent', 'rol_merent', 'rol_vac_chg', 'rol_G_mrent','rol_G_merent', 'qrol_vac', 'qrol_mrent', 'qrol_merent', 'vac_new', 'mrent_new', 'merent_new', 'v_user', 'g_user', 'e_user', 'vac_oob', 'vac_chg_oob', 'mrent_oob', 'G_mrent_oob', 'merent_oob', 'G_merent_oob']]
            ...
            rebench_log['init_shim_period_vac'] = np.where(rebench_log['vac_diff'].isnull() == True, np.nan, rebench_log['init_shim_period_vac'])
            rebench_log['init_shim_period_mrent'] = np.where(rebench_log['mrent_diff'].isnull() == True, np.nan, rebench_log['init_shim_period_mrent'])
            rebench_log['init_shim_period_merent'] = np.where(rebench_log['merent_diff'].isnull() == True, np.nan, rebench_log['init_shim_period_merent'])
            rebench_log['v_user'] = np.where(rebench_log['vac_diff'].isnull() == True, np.nan, rebench_log['v_user'])
            rebench_log['g_user'] = np.where(rebench_log['mrent_diff'].isnull() == True, np.nan, rebench_log['g_user'])
            rebench_log['e_user'] = np.where(rebench_log['merent_diff'].isnull() == True, np.nan, rebench_log['e_user'])
            ...
            rebench_log['vac_diff_max'] = rebench_log.groupby('identity')['abs_vac_diff'].transform('max')
            rebench_log['mrent_diff_max'] = rebench_log.groupby('identity')['abs_mrent_diff'].transform('max')
            rebench_log['merent_diff_max'] = rebench_log.groupby('identity')['abs_merent_diff'].transform('max')
            rebench_log['vac_diff'] = np.where(rebench_log['vac_diff'] < 0, rebench_log['vac_diff_max'] * -1, rebench_log['vac_diff_max'])
            rebench_log['mrent_diff'] = np.where(rebench_log['mrent_diff'] < 0, rebench_log['mrent_diff_max'] * -1, rebench_log['mrent_diff_max'])
            rebench_log['merent_diff'] = np.where(rebench_log['merent_diff'] < 0, rebench_log['merent_diff_max'] * -1, rebench_log['merent_diff_max'])
            ...
            rebench_log = rebench_log.drop(['vac_diff_max', 'mrent_diff_max', 'merent_diff_max'], axis=1)
            rebench_log = rebench_log.drop(['rol_vac', 'rol_mrent', 'rol_merent', 'rol_vac_chg', 'rol_G_mrent',	'rol_G_merent'], axis=1)
...
def output_display(sector_val, drop_val, all_buttons, key_met_val, expand, show_cd, show_skips, has_flag, flag_list, p_skip_list, orig_cols, curryr, currmon, flags_resolved, flags_unresolved, flags_new, flags_skipped, success_init, flag_cols, init_skips, init_comment_cons, init_comment_avail, init_comment_mrent, init_comment_erent, ncsur_props, surv_avail_props, all_avail_props, surv_rg_props, all_rg_props, newnc_props, test_auto_rebench, message, first_yr, first_month, auto_rebench_var):
    ...
    if sector_val is None or success_init == False:
        ...
        if "full" not in expand:
            data = data[(data['yr'] == curryr) | ((data['yr'] == curryr - 1) & ((data['currmon'] >= priormon) | (currmon == 1))) | ((data['yr'] == curryr - 2) & (data['currmon'] == 12) & (currmon == 1))]
            if len(preview_data) > 0:
                preview_data = preview_data[(preview_data['yr'] == curryr) | ((preview_data['yr'] == curryr - 1) & ((preview_data['currmon'] >= priormon) | (currmon == 1))) | ((data['yr'] == curryr - 2) & (data['currmon'] == 12) & (currmon == 1))]
            if len(shim_data) > 0:
                shim_data = shim_data[(shim_data['yr'] == curryr) | ((shim_data['yr'] == curryr - 1) & ((shim_data['currmon'] >= priormon) | (currmon == 1))) | ((shim_data['yr'] == curryr - 2) & (shim_data['currmon'] == 12) & (currmon == 1))]
        ...
        if "full" in expand and not first_yr and (shim_data.reset_index().loc[0]['yr'] == curryr or ((shim_data.reset_index().loc[0]['yr'] == curryr - 1) and (shim_data.reset_index().loc[0]['currmon'] == currmon))):
            shim_add = data.copy()
